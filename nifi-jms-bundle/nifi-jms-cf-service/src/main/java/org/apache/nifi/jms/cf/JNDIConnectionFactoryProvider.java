...
/**
 * Provides a factory service that creates and initializes {@link ConnectionFactory} specific to the third party JMS
 * system.
 * <p>
 * It accomplishes it by adjusting current classpath by adding to it the additional resources (i.e., JMS client
 * libraries) provided by the user via {@link JMSConnectionFactoryProviderDefinition#CLIENT_LIB_DIR_PATH}, allowing it
 * then to create an instance of the target {@link ConnectionFactory} based on the provided
 * {@link JMSConnectionFactoryProviderDefinition#CONNECTION_FACTORY_IMPL} which can be than access via
...
public class JNDIConnectionFactoryProvider extends AbstractControllerService implements JMSConnectionFactoryProviderDefinition {
    ...
    /**
     * Sets corresponding {@link ConnectionFactory}'s property to a 'propertyValue' by invoking a 'setter' method that
     * corresponds to 'propertyName'. For example, 'channel' property will correspond to 'setChannel(..) method and
     * 'queueManager' property will correspond to setQueueManager(..) method with a single argument.
     *
     * NOTE: There is a limited type conversion to accommodate property value types since all NiFi configuration
     * properties comes as String. It is accomplished by checking the argument type of the method and executing its
     * corresponding conversion to target primitive (e.g., value 'true' will go thru Boolean.parseBoolean(propertyValue)
     * if method argument is of type boolean). None-primitive values are not supported at the moment and will result in
     * {@link IllegalArgumentException}. It is OK though since based on analysis of several ConnectionFactory
     * implementation the all seem to follow bean convention and all their properties using Java primitives as
     * arguments.
     */
    ...
    private boolean isSolace(ConfigurationContext context) {
        String connectionFactoryImplName = context.getProperty(CONNECTION_FACTORY_IMPL).evaluateAttributeExpressions().getValue();
        return connectionFactoryImplName != null && connectionFactoryImplName.startsWith("com.solacesystems.jndi");
    }
    ...
    /**
     * Will convert propertyName to a method name following bean convention. For example, 'channel' property will
     * correspond to 'setChannel method and 'queueManager' property will correspond to setQueueManager method name
     */
